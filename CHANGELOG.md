...
- 新增基于 Qwen CLI 的 ACP stdio 连接链路，支持在桌面端直接驱动 Qwen Agent 会话。
- 新增 Qwen 历史会话导入能力，可从 `~/.qwen/projects/<workspace-key>/chats/*.jsonl` 读取历史记录并映射到当前工作区。
- 新增工作区文件列表面板，可在右上角打开当前目录文件清单并将路径快速插入会话输入框。
- 新增长任务活动状态提示：
  - 输入框下方实时显示最近活动时间、当前阶段与最近工具；
  - “更多 -> 工具”面板顶部同步显示活跃摘要，用于判断任务是否仍在推进。

### Changed

- 桌面端 Agent 驱动从 iFlow CLI 全量切换到 Qwen CLI，连接方式从 WebSocket ACP 调整为 stdio ACP。
- 版本号提升到 `1.0.0`，作为 Qwen 驱动版本的首个正式发布版本。
- 模型与能力元数据改为优先依赖 ACP 运行时返回，不再依赖旧的 iFlow 静态解析链路。
- 历史会话来源、上下文占用显示与自动重连流程已适配 Qwen 会话模型。
- 创建 Agent、默认路径、界面提示等对外文案统一调整为 Qwen 语义。

### Fixed

- 修复 HTML Artifact 预览偶发把“文件已保存到：...”整段文本误当作路径，导致预览失败的问题。
- 修复 Qwen 低上下文占比场景显示过粗的问题，低占比时改为保留一位小数并优先采用 Qwen 实际 usage。
- 修复历史会话列表中同一 Qwen 会话被渲染成两条记录的问题，增加 `local` 与 `qwen-log` 的自动合并逻辑。
- 修复“重连 Agent”单击无响应的问题，统一使用当前 Qwen 重连链路。
- 修复长任务完成前缺少可见运行信号的问题，减少“是否卡住”的判断盲区。
...
- 序列化消息时剔除 `estimatedTokens` 缓存字段，避免持久化噪音。
...
- 新增发送快捷键设置，支持在 `Enter` 与 `⌘+Enter/Ctrl+Enter` 之间切换，适配不同用户习惯。
- 优化 IME 输入法组合状态检测，避免中文输入时误触发发送。

## [0.3.9] - 2026-03-11

### Added

- 新增 Vitest 测试基础与 `npm test` 脚本，补齐最小单测运行能力。
- 新增自动重连与模型归一化相关测试用例，覆盖核心逻辑。
...
- 新增“对话完成提示音”能力：在会话 `task-finish` 后自动播放提醒。
- 新增顶部铃声选择器，支持多铃声切换与“关闭铃声”选项。
- 新增项目内铃声素材目录 `public/audio/bell`，用于前端静态资源播放。

### Changed

- 铃声素材文件名统一为简洁命名（如 `bell-happy.wav`、`chime-quick.wav`），便于维护与引用。
- 优化应用图标视觉体积，调整到更接近系统图标的 Dock 显示比例。
- 更新版本号到 `v0.3.6`，同步前端、Rust、Tauri 配置与项目文档版本信息。
...
- 思考开关在不支持场景显示为 `思考：不支持`，并提供明确提示信息。
- 更新版本号到 `v0.3.5`，同步前端、Rust、Tauri 配置与项目文档版本信息。
...
- 文件 Diff 弹窗标题改为单行 `文件 <path> Diff`，并增大弹窗宽度以提升可读性。
- 文件变更刷新体验优化：增加最后刷新时间，并在 Agent 忙碌时禁用刷新按钮。
- 更新版本号到 `v0.3.4`，同步前端、Rust、Tauri 配置与项目文档版本信息。
...
- 新增开发环境清理脚本 `scripts/clean-dev.sh`，用于启动前清理 1420 端口监听与当前项目调试残留进程。

### Changed

- `tauri:dev` 启动链路调整为先执行 `npm run clean` 再启动 Tauri，减少开发阶段端口冲突。
- Agent 进程销毁链路统一，`disconnect/switch` 与应用退出事件都复用同一套回收逻辑。
- 更新版本号到 `v0.3.3`，同步前端、Rust、Tauri 配置与项目文档版本信息。

### Fixed

- 修复开发阶段关闭后再次运行偶发提示 `1420` 端口占用的问题。
- 修复应用退出时当前实例 Agent 子进程可能残留的问题。

## [0.3.2] - 2026-02-28

### Added

- 新增应用图标资源（基于 `src-tauri/icons/Subject.png` 生成），补齐 Tauri 打包所需图标集。
- README 新增 Icon 标识与图标展示，便于快速识别当前应用图标。

### Changed

- 更新版本号到 `v0.3.2`，同步前端、Rust、Tauri 配置与项目文档版本信息。
