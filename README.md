...
- 上下文占用提示：输入区展示进度条与百分比，点击可触发压缩（估算）
- 上下文估算：按模型映射上下文窗口并基于消息内容估算 Token 占用
- 测试补齐：新增 Token 估算与模型上下文映射单测
- 发送快捷键：支持在 `Enter` 与 `⌘+Enter/Ctrl+Enter` 之间切换，优化中文输入法场景
- 测试基础：引入 Vitest 与 `npm test` 脚本，补齐最小单测能力
- 自动重连：重连成功后刷新输入框状态，避免界面卡在旧状态
- 模型解析：统一模型选项归一化逻辑，兼容非字符串元数据
- 离线输入：Agent 离线时禁用输入区并提示先连接
